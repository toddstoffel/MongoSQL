...
class JSONFunctionMapper:
    ...
    def map_json_function(
        self,
        function_name: str,
        args: List[Any] = None,
        function_token: Function = None,
    ) -> Dict[str, Any]:
        """
    ...
    def _create_operation_from_args(
        self, function_name: str, args: List[Any]
    ) -> Optional[JSONOperation]:
        ...
        # Validate arguments
        if not mapping.validate_arguments(processed_args):
            return None
        ...
        if mapping.supports_path and len(processed_args) >= 2:
            operation.json_path = self.parser._parse_json_path(processed_args[1])
    ...
    def get_function_examples(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "JSON_KEYS": ["JSON_KEYS(user_data)", "JSON_KEYS(user_data, '$.address')"],
            "JSON_LENGTH": [
