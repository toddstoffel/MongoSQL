...
class OrderByParser:
    ...
    def _parse_with_tokens(self, sql: str) -> Optional[OrderByClause]:
        ...
        try:
            ...
            # Track parentheses depth to avoid ORDER BY inside OVER clauses
            paren_depth = 0
            ...
            for i, token in enumerate(tokens):
                ...
                # Look for "ORDER BY" but only at top level (not inside parentheses)
                if token_upper == "ORDER BY" and paren_depth == 0:
                    order_by_found = True
                    continue
                elif order_by_found:
