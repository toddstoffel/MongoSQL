...
class EnhancedAggregateTranslator:
    ...
    def translate_operations(
        self,
        operations: List[EnhancedAggregateOperation],
    ...
    def _add_group_concat_stages(
        ...
        for operation in operations:
            ...
            # Use the proper MongoDB aggregation method that handles ORDER BY
            pipeline.extend(group_concat.to_mongodb_aggregation())

        return pipeline

    def _add_statistical_stages(
        self,
        pipeline: List[Dict[str, Any]],
        operations: List[EnhancedAggregateOperation],
    ) -> List[Dict[str, Any]]:
        ...
        if group_stage_index >= 0:
            ...
            for operation in operations:
                ...
                group_stage["$group"][field_name] = (
                    self.get_statistical_function_expression(
                        func.function_type, func.field
                    )
                )
        else:
            ...
            for operation in operations:
                func = operation.function
                field_name = f"{func.function_type.value.lower()}_{func.field}"
                group_fields[field_name] = self.get_statistical_function_expression(
                    func.function_type, func.field
                )

            pipeline.append({"$group": {"_id": None, **group_fields}})

        return pipeline

    def _add_bitwise_stages(
        self,
        pipeline: List[Dict[str, Any]],
        operations: List[EnhancedAggregateOperation],
    ) -> List[Dict[str, Any]]:
        ...
        if group_stage_index >= 0:
            ...
            for operation in operations:
                ...
                field_name = f"{func.function_type.value.lower()}_{func.field}"
                group_stage["$group"][field_name] = (
                    self.get_bitwise_function_expression(func.function_type, func.field)
                )
        else:
            # Create new group stage
            group_fields = {}
            for operation in operations:
                func = operation.function
                field_name = f"{func.function_type.value.lower()}_{func.field}"
                group_fields[field_name] = self.get_bitwise_function_expression(
                    func.function_type, func.field
                )
    ...
    def get_statistical_function_expression(
        self, func_type: EnhancedAggregateFunctionType, field: str
    ) -> Dict[str, Any]:
    ...
    def get_bitwise_function_expression(
        self, func_type: EnhancedAggregateFunctionType, field: str
    ) -> Dict[str, Any]:
