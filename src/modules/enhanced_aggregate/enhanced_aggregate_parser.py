...
class EnhancedAggregateParser:
    ...
    def _extract_from_select(self, parsed) -> List[EnhancedAggregateOperation]:
        ...
        for token in parsed.flatten():
            if token.ttype is sql.DML and token.value.upper() == 'SELECT':
                select_found = True
                continue
            
            if select_found and token.ttype is sql.Keyword and token.value.upper() in ['FROM', 'WHERE', 'GROUP', 'ORDER', 'HAVING', 'LIMIT']:
                break
            ...
            if select_found and hasattr(token, 'value'):
                ...
                if func_match:
                    operation = self._parse_function_call(func_match, 'SELECT')
                    if operation:
    ...
    def _extract_from_having(self, parsed) -> List[EnhancedAggregateOperation]:
        ...
        having_found = False
        for token in parsed.flatten():
            if token.ttype is sql.Keyword and token.value.upper() == 'HAVING':
                having_found = True
                continue
                
            if having_found and token.ttype is sql.Keyword and token.value.upper() in ['ORDER', 'LIMIT']:
                break
    ...
    def _extract_function_from_token(self, token_value: str) -> Optional[Dict[str, str]]:
    ...
    def _parse_function_call(self, func_info: Dict[str, str], context: str) -> Optional[EnhancedAggregateOperation]:
        ...
        if func_name == 'GROUP_CONCAT':
            ...
            return self._parse_statistical_function(func_name, arguments, full_call, context)
        elif func_name in ['BIT_AND', 'BIT_OR', 'BIT_XOR']:
            return self._parse_bitwise_function(func_name, arguments, full_call, context)
        
        return None
    
    def _parse_group_concat(self, arguments: str, full_call: str, context: str) -> Optional[EnhancedAggregateOperation]:
        """Parse GROUP_CONCAT function with all its options"""
        # Remove outer parentheses
        args_content = arguments.strip()[1:-1] if arguments.startswith('(') and arguments.endswith(')') else arguments
        ...
        if 'DISTINCT' in args_content.upper():
            distinct = True
            args_content = re.sub(r'\bDISTINCT\b', '', args_content, flags=re.IGNORECASE).strip()
        
        # Check for SEPARATOR clause
        separator_match = re.search(r'\bSEPARATOR\s+[\'"]([^\'"]*)[\'"]', args_content, re.IGNORECASE)
        if separator_match:
            separator = separator_match.group(1)
            args_content = re.sub(r'\bSEPARATOR\s+[\'"][^\'"]*[\'"]', '', args_content, flags=re.IGNORECASE).strip()
        
        # Check for ORDER BY clause
        order_match = re.search(r'\bORDER\s+BY\s+(.+?)(?:\s+SEPARATOR|$)', args_content, re.IGNORECASE)
        if order_match:
            order_clause = order_match.group(1).strip()
            order_by = self._parse_order_by_clause(order_clause)
            args_content = re.sub(r'\bORDER\s+BY\s+.+?(?:\s+SEPARATOR|$)', '', args_content, flags=re.IGNORECASE).strip()
        ...
        return EnhancedAggregateOperation(
            function=group_concat_func,
            context=context
        )
    
    def _parse_statistical_function(self, func_name: str, arguments: str, full_call: str, context: str) -> Optional[EnhancedAggregateOperation]:
        ...
        return EnhancedAggregateOperation(
            function=statistical_func,
            context=context
        )
    
    def _parse_bitwise_function(self, func_name: str, arguments: str, full_call: str, context: str) -> Optional[EnhancedAggregateOperation]:
