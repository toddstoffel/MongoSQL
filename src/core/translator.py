...
class MongoSQLTranslator:
    ...
    def _translate_select(self, parsed_sql: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if parsed_sql.get("distinct"):
            ...
            if len(columns) == 1 and columns[0] != "*":
                ...
                field_name = (
                    ...
                    else columns[0].get("column", columns[0])
                )
        ...
        mql = {"operation": "find", "collection": parsed_sql.get("from")}
            ...
            if parsed_sql.get("order_by"):
                ...
                if original_sql:
                    ...
                    if order_by_clause and not order_by_clause.is_empty():
                        ...
                        sort_stages = self.orderby_translator.get_sort_pipeline_stage(
                            order_by_clause
                        )
                        if sort_stages:
    ...
    def _translate_distinct_multiple(
        ...
        raise Exception(
            "Multi-column DISTINCT not yet supported. Use single column DISTINCT."
        )

    def _translate_distinct_with_limit(
        self, parsed_sql: Dict[str, Any], field_name: str
    ) -> Dict[str, Any]:
        ...
        if parsed_sql.get("where"):
            pipeline.append({"$match": self._translate_where(parsed_sql["where"])})
        ...
        pipeline.append({"$project": {"_id": 0, field_name: "$_id"}})
    ...
    def _translate_insert(self, parsed_sql: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(values) == 1:
            ...
            if columns:
                ...
                raise Exception(
                    "INSERT without column specification requires schema information"
                )
    ...
    def _translate_delete(self, parsed_sql: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mql = {"operation": "delete_many", "collection": table}  # Default to deleteMany
        ...
        if value and isinstance(value, list):
            for item in value:
                if isinstance(item, str) and (
                    "SELECT" in item.upper() or "(SELECT" in item
                ):
                    return (
                        {}
                    )  # Return empty filter, let subquery translator handle this
    ...
    def _translate_compound_where(self, where_clause: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for condition in conditions:
            ...
            if field and operator:
                mongo_condition = self._translate_single_condition(
    ...
    def _translate_single_condition(
        ...
        if operator == "=":
            ...
            if isinstance(value, list):
                ...
                in_values = [
                    v.strip().strip("'\"") for v in value.strip("()").split(",")
                ]
    ...
    def _translate_show(self, parsed_sql: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if show_type == "TABLES":
            return {"operation": "show_collections", "collection": None}
        elif show_type == "DATABASES":
            return {"operation": "show_databases", "collection": None}
        else:
    ...
    def _translate_use(self, parsed_sql: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"operation": "use_database", "database": database_name}

    def _handle_aggregate_functions(
        self, parsed_sql: Dict[str, Any]
        ...
        if group_by:
            group_by_structure = self.groupby_parser.parse_group_by_structure(
                parsed_sql
            )
        ...
        if len(aggregate_functions) == 1 and not regular_columns:
            ...
            if func_name == "COUNT":
                ...
                original_call = func_info.get(
                    "original_call", f"GROUP_CONCAT({func_arg})"
                )
                return self._translate_group_concat_aggregate(
                    parsed_sql, func_arg, original_call
                )
            elif func_name in ["STDDEV_POP", "STDDEV_SAMP", "VAR_POP", "VAR_SAMP"]:
                return self._translate_statistical_aggregate(
                    parsed_sql, func_name, func_arg
                )
            elif func_name in ["BIT_AND", "BIT_OR", "BIT_XOR"]:
                return self._translate_bitwise_aggregate(
                    parsed_sql, func_name, func_arg
                )
    ...
    def _translate_count_aggregate(
        self, parsed_sql: Dict[str, Any], arg: str
    ) -> Dict[str, Any]:
        """Translate COUNT aggregate to MongoDB count operation"""
        mql = {"operation": "count", "collection": parsed_sql.get("from")}
    ...
    def _translate_math_aggregate(
        self, parsed_sql: Dict[str, Any], func_name: str, field: str
    ) -> Dict[str, Any]:
    ...
    def _translate_group_concat_aggregate(
        self, parsed_sql: Dict[str, Any], field: str, original_call: str = None
    ) -> Dict[str, Any]:
        """Translate GROUP_CONCAT to MongoDB aggregation pipeline"""
        if field == "*":
            raise Exception("GROUP_CONCAT requires a specific field, not *")

        # Parse the field to extract ORDER BY clause, DISTINCT, and SEPARATOR if present
        actual_field = field
        order_by_field = None
        order_direction = 1  # Default ASC
        use_distinct = False
        ...
        if "SEPARATOR" in field.upper():
            ...
            # Extract everything before SEPARATOR
            field_before_separator = field[:separator_pos].strip()

            # Extract the separator value (everything after SEPARATOR)
            separator_clause = field[
                separator_pos + 9 :
            ].strip()  # 9 = len('SEPARATOR')

            # Parse the separator value (remove quotes)
            if separator_clause.startswith("'") and separator_clause.endswith("'"):
                separator = separator_clause[1:-1]
            elif separator_clause.startswith('"') and separator_clause.endswith('"'):
                separator = separator_clause[1:-1]
            else:
                # Handle separator without quotes
                separator = separator_clause

            # Continue parsing with the part before SEPARATOR
            field = field_before_separator

        # Check if field contains ORDER BY clause
        if "ORDER BY" in field.upper():
            # Find the ORDER BY position
            order_by_pos = field.upper().find("ORDER BY")

            # Extract the actual field (everything before ORDER BY)
            actual_field = field[:order_by_pos].strip()

            # Extract the ORDER BY clause (everything after ORDER BY)
            order_clause = field[order_by_pos + 8 :].strip()  # 8 = len('ORDER BY')

            # Parse ORDER BY field and direction
            order_parts = order_clause.split()
            if order_parts:
                order_by_field = order_parts[0]
        ...
        # Add sort stage if ORDER BY is specified
        if order_by_field:
            pipeline.append({"$sort": {order_by_field: order_direction}})
        ...
        if original_call:
            field_display = original_call
        elif "ORDER BY" in field.upper() or "SEPARATOR" in field.upper():
            field_display = field
        else:
            field_display = actual_field
    ...
    def _translate_statistical_aggregate(
        self, parsed_sql: Dict[str, Any], func_name: str, field: str
    ) -> Dict[str, Any]:
    ...
    def _translate_bitwise_aggregate(
        self, parsed_sql: Dict[str, Any], func_name: str, field: str
    ) -> Dict[str, Any]:
        ...
        mongo_op_map = {"BIT_AND": "$bitAnd", "BIT_OR": "$bitOr", "BIT_XOR": "$bitXor"}
        ...
        pipeline.append({"$group": {"_id": None, "values": {"$push": f"${field}"}}})
        ...
        initial_values = {
            ...
            },  # Start with first value for AND
    ...
    def _handle_no_table_query(self, parsed_sql: Dict[str, Any]) -> Dict[str, Any]:
                    ...
                    if "args" in col:
                        ...
                        for arg in args:
                            ...
                            if self._contains_expression(arg):
                                ...
                                try:
                                    evaluated_arg = self._evaluate_argument_expression(
                                        arg
                                    )
                    ...
                    if " as " in col_stripped.lower():
                        ...
                        if as_pos != -1:
                            ...
                            try:
                                result = eval(
                                    expression_part
                                )  # Simple math expressions like "1"
        ...
        return {"operation": "eval", "type": "no_table_query", "projection": projection}
    ...
    def _handle_case_when_with_from(
        self, case_expression: dict, parsed_query: dict
    ) -> dict:
        ...
        for when_clause in case_expression.get("when_clauses", []):
            ...
            if condition == "1=1":
                mongo_condition = {
                    "$literal": True
                }  # Use $literal for boolean expressions
            else:
            ...
            branch = {"case": mongo_condition, "then": mongo_value}
        ...
        switch_expression = {"branches": switch_branches, "default": default_value}

        # Add $project stage with the $switch expression
        field_name = "result"
        project_stage = {
            "$project": {"_id": 0, field_name: {"$switch": switch_expression}}
        }
    ...
    def _translate_with_regexp_support(
        self, parsed_sql: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _handle_no_table_regexp_query(
        self, parsed_sql: Dict[str, Any], enhanced_columns: List[Any]
    ) -> Dict[str, Any]:
        ...
        for col in enhanced_columns:
            if isinstance(col, dict) and col.get("type") == "regexp_expression":
                ...
                try:
                    ...
                    regexp_result = regexp_mapper.translate_infix_regexp(
                        expr, "SELECT", alias
                    )
                    if regexp_result:
