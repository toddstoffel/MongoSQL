...
class MongoDBClient:
    ...
    def __init__(
        ...
        username: str = None,
        password: str = None,
        retry_writes: str = "true",
        write_concern: str = "majority",
        app_name: str = "MongoSQL",
    ):
    ...
    def connect(self):
        ...
        if (
            MongoDBClient._client is not None
            and MongoDBClient._connection_params == current_params
        ):
        ...
        try:
            ...
            if self.database_name and (self.username or self.password):
                ...
                if self.database_name not in all_db_names:
                    raise Exception(
                        f"ERROR 1049 (42000): Unknown database '{self.database_name}'"
                    )
            ...
            raise Exception(
                f"ERROR 2003 (HY000): Can't connect to MongoDB server on '{self.host}' ({e})"
            )
            ...
            if e.code == 18:  # Authentication failed
                ...
                raise Exception(
                    f"ERROR 1045 (28000): Access denied for user '{self.username}'@'{hostname}' (using password: YES)"
                )
            else:
                raise Exception(
                    f"ERROR 1045 (28000): Access denied for user '{self.username}'@'{self.host}' (using password: YES)"
                )
    ...
    def execute_query(self, mql_query: Dict[str, Any]) -> Any:
        ...
        if operation == "show_collections":
            ...
            # Format as list of dictionaries to match table display
            table_name = (
                f"Tables_in_{self.database_name}" if self.database_name else "Tables"
            )
        ...
        try:
            if operation == "find":
                ...
                pipeline = self._add_implicit_ordering_for_deterministic_results(
                ...
                has_sort = any(
                    "$sort" in stage for stage in pipeline if isinstance(stage, dict)
                )
            ...
            if e.code == 26:  # NamespaceNotFound
                raise Exception(
                    f"ERROR 1146 (42S02): Table '{collection_name}' doesn't exist"
                )
    ...
    def _evaluate_expression(self, expression: Dict[str, Any]) -> Any:
        ...
        if "$dateToString" in expression:
            ...
            if isinstance(values, list):
                try:
                    ...
                    if all(
                        ...
                        or (
                            isinstance(v, dict)
                            and isinstance(self._evaluate_expression(v), int)
                        )
                        for v in values
                    ):
                        return int(result)
            ...
            # Switch expression (case-when)
            switch_expr = expression["$switch"]

            if isinstance(switch_expr, dict):
                branches = switch_expr.get("branches", [])
                default = switch_expr.get("default")
                ...
                for branch in branches:
                    if isinstance(branch, dict):
                        ...
                        if isinstance(case_expr, dict):
                            condition_result = self._evaluate_expression(case_expr)
                        else:
                            condition_result = bool(case_expr)

                        # If condition is true, return the then value
                        if condition_result:
                            if isinstance(then_expr, dict):
                                return self._evaluate_expression(then_expr)
                            return then_expr

                # If no branch matched, return default
                if default is not None:
                    if isinstance(default, dict):
                        return self._evaluate_expression(default)
                    return default
            ...
            try:
                ...
                days = (
                    365 * year + (year - 1) // 4 - (year - 1) // 100 + (year - 1) // 400
                )
    ...
    def _add_implicit_ordering_for_deterministic_results(
        ...
        if has_limit and not has_sort:
            ...
            if limit_index >= 0:
                ...
                sort_stage = {
                    "$sort": {implicit_sort_field: 1}
                }  # ASC like MariaDB primary key
